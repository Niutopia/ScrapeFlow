...
>
> 长期产品与工程合同见 [`../AGENTS.md`](../AGENTS.md)；当前事实以根 AGENTS.md、源码、测试和 Git 工作树证据核对。其中 target-shelf-first 入口仅是本历史计划的当时方案，
> 已被长期合同中的 reconciliation-first 流程取代。

在当时的计划中，项目“完成”的定义是：普通入库、归档、审计、严格三阶补源、恢复、备份和本机部署全部闭环。新的 Web 不在本轮完成门内，未来只消费稳定 API。
