...
| 4 | golden path passed | `local.tests.test_phase4_golden_path`；普通视频、ZIP/7z/RAR、EXE/BIN/DAT 伪装、字幕 sidecar 及负例 fixture | source 成功移入 task-owned processed；失败保留 source；重启不重复 writer |
...
- 当前工作树仍是混合 WIP，本文不表示已经完成逐阶段提交或真实媒体库发布。
