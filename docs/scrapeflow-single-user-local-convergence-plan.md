...
>
> 产品目标来源：[ScrapeFlow 自动工作流规格与路线图](./scrapeflow-unified-workflow-remediation-plan.md)

## 1. 计划目的

本计划保留原有任务文档的产品方向：用户提交一个来源目录，系统自动完成身份识别、规划、写入、审计、补源和任务自有临时内容清理。
...
- 一个 AList 实例和一套媒体库根。
- 一个正式库 writer。
- Provider worker 默认保持少量并发，不追求吞吐最大化。
...
以下数字只是 2026-08-08 的实施起点，不是运行时权威；实时状态仍以 Web 和 API 为准。
...
普通入站归档和 Provider SFX 共用同一归档模块，解出后回到同一 Engine 规划和单 writer 路径。

#### 统一流程

```text
入站来源 / Provider payload
...
1. 普通来源在 TMDB/媒体规划前增加 archive preprocessing。
...
7. 成功完成正式写入后才能清理该任务拥有的原归档和 staging。
