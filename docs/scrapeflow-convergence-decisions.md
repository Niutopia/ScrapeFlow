...
本文保留 2026-08-09 之前收敛工作的安全与恢复决定；它不再是普通入站入口规则的唯一来源，也不声明这些规则已经在当前工作树完成实现。普通来源必须先登记为 `awaiting_target_shelf`，由用户选择固定一级货架并通过 `/start` 启动；当前权威状态机、实施顺序和完成门见[用户选择目标货架后启动实施计划](./scrapeflow-target-shelf-start-gate-plan.md)。
