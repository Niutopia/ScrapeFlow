...
7. **T 终态消费源树与展开 staging（用户裁决 2026-08-27，扩展 2026-09-02）**:
   ...
   - **未映射视频闸门（用户裁决 2026-09-05）**：消费时源树里仍在的每个视频，若其集坐标已被本根验收的正式库集号覆盖（库已覆盖坐标 = 规划器质量裁决的落选版本），按「只留高版本」直接随源删除，不进待裁决；其余视频若主题命名（NCOP/NCED/OP/ED/MENU/PV/CM）、bonus 目录内容、探测时长 < 5 分钟且无正片语法，判垃圾随源删除；**再其余一切（无号 SP、未放送话、web 限定特别篇、正片命名、时长 ≥ 5 分钟、或探测失败判不出的）一律不删**，move 至 `/ScrapeFlow/待裁决/<源目录名>/`（操作员在待刮削建的文件夹名；同名旧根残留时后缀短任务号消歧）**平铺**（不镜像源目录结构；同名基名以完整相对父链消毒+摘要后缀消歧子目录防覆盖）并写 manifest（文件名/大小/时长[机器可读 duration_seconds]/原因/原始路径），配对外挂字幕随行；正片级时长（≥ 5 分钟）凌驾一切命名与目录信号——藏在特典目录里的未放送话同样隔离。引擎无法证明 TMDB 未收录的特别篇没有价值，证明不了就不删；
