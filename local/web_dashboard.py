...
.cleanup-btn{margin-top:7px;border:1px solid #454138;background:transparent;color:#8a867e;font-size:8px;padding:2px 7px;cursor:pointer}
.cleanup-btn:hover{color:#d89b76;border-color:#5a3b2f}
...
const shelfLabels={movie:'电影',anime:'番剧',us_tv:'美剧'},terminal=new Set(['completed','completed_with_gaps','executed','cancelled']),attention=new Set(['failed','failed_archive','failed_cleanup','failed_identity','failed_planning','failed_provider','failed_verification','failed_write','reconciliation_uncertain','target_policy_conflict']);
...
function title(j){const raw=j.plan?.title||j.source||j.id;return String(raw).split('/').filter(Boolean).pop()||j.id}
function sub(j){return j.source==='全库审计'?j.id:(j.source||j.id)}
function phaseLabel(p){return({queued:'已排队',planned:'已计划',reconciling:'正在对账',reconciled:'已对账',needs_attention:'需要关注',archive_preprocessing:'归档预处理',identity_matching:'识别内容',planning:'生成计划',executing:'正在写入',executing_media:'正在写入',verifying:'正在核验',cleaning:'清理暂存',completed:'已完成',completed_with_gaps:'已完成（有缺口）',executed:'已执行',cancelled:'已取消',failed:'失败',failed_archive:'归档失败',failed_cleanup:'清理失败',failed_identity:'识别失败',failed_planning:'计划失败',failed_provider:'补源失败',failed_verification:'验收失败',failed_write:'写入失败',reconciliation_uncertain:'对账不确定',target_policy_conflict:'货架冲突',retry_wait:'等待重试',awaiting_target_shelf:'等待选择货架',gap_discovering:'核对缺口',provider_searching:'搜索补源',acquiring:'获取文件',staging_verifying:'核对暂存',subtitle_installing:'安装字幕',child_planning:'规划补源',child_executing:'整理补源',final_verifying:'最终核对'}[p]||p)}
...
$('#jobList').onclick=e=>{const b=e.target.closest('[data-cleanup]');if(!b)return;if(!window.confirm('删除这条任务记录？只清理本地任务状态，绝不触碰媒体文件。'))return;post(`/api/jobs/${encodeURIComponent(b.dataset.cleanup)}/cleanup`,{})};
